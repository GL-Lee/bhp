var server = "http://ptest.tianqi.com:3000/";
var weatherPath = server + "weather";
var cityCode = "";
if(cityCode){
	weatherPath = server + "weather?version=%s&source=%s&uuid=%s&cityCode="+cityCode;
}
...
var model = {
	...
	init: function(){
		...
		if(this.cityInfo){
			this.userInfo = {
				...
				source: 'chrome',
				uuid: '100',
				cityCode: this.cityInfo.xian.code
			}
		}
		this.getWeatherData(this.userInfo);
		// this.getWeatherData({}, this.view.renderTrend);
		// this.getCitys(this.view.renderSet);
	},
    getWeatherData: function(userInfo) {
        var _this = this;
    ...
    dataWrap: function(weatherData){
    	function tempConvert(temp){
    		var nTemps = temp.replace("℃", "").split("~");
    		return {highTemp: parseInt(nTemps[0]),lowTemp: parseInt(nTemps[1])};
    	}
...
var view = {
	...
	init: function(){
		...
		var noticeTime = model.storage.get("notice-time", true);
		if(noticeTime && (noticeTime[0].on || noticeTime[1].on)){
			clearInterval(gInterval);
			gInterval = setInterval(function(){
				notificationShow(noticeTime)
			}, 10000);
		}
	},
	bindEvents: function(){
		...
		function bindEvents(){
			$set.find(".selectCity .button").bind("click",function(){
				...
				if(!_cityInfo || (_cityInfo && _cityInfo.xian.code != cityInfo.xian.code)){
					...
					view.model.getWeatherData(view.model.userInfo);
...
function getOpts() {
	var weatherinfo = model.weatherData.weatherinfo;
	var date = new Date(model.weatherData.time);
	var am_pm = date.getHours() < 18 ? 'AM' : 'PM';
	var iconUrlBase = "http://hao.weidunewtab.com/tianqi/images/weather/";
	var iconNum1 = weatherinfo.img1["img"+am_pm];
	if(iconNum1 == 99){
		iconNum1 = weatherinfo.img1.imgAM;
	}
	return {
		type : "basic",
		title : weatherinfo.city + "天气",
		message : "今天：" + weatherinfo.temp1 + " " + weatherinfo.weather1 + "\r\n"
				+ weatherinfo.wind1["wind"+am_pm]+ weatherinfo.wind1["windSize"+am_pm],
		iconUrl : iconUrlBase+iconNum1+".png",
		buttons : [{
			title : "明天：" + " " + weatherinfo.temp2 + " " + weatherinfo.weather2 + ","
					+ weatherinfo.wind2["wind"+am_pm]+ weatherinfo.wind2["windSize"+am_pm],
			iconUrl : "images/weather/" + weatherinfo.img2.imgAM + ".png"
		}, {
			title : "后天：" + " " + weatherinfo.temp3 + " " + weatherinfo.weather3 + ","
					+ weatherinfo.wind3["wind"+am_pm]+ weatherinfo.wind3["windSize"+am_pm],
			iconUrl : "images/weather/" + weatherinfo.img3.imgAM + ".png"
		}]
	}
}
function notificationShow(noticeTime) {
	var date = new Date();
	var hour = date.getHours();
	var minute = date.getMinutes();
	if((noticeTime[0].on && hour == noticeTime[0].hour.substr(0,2) && minute == noticeTime[0].minute.substr(0,2)) ||
		(noticeTime[0].on &&  hour == noticeTime[1].hour.substr(0,2) && minute == noticeTime[1].minute.substr(0,2))){
		var nObj = getOpts();
		chrome.notifications.create("", nObj, function() {})
		gNotificationShowed = true;
		clearInterval(gInterval)
	}
}
